...
builder.UseNativeStyles(options =>
{
    options.Brand = new BrandPalette(Color.FromArgb("#0B7A75"), accentDark: Color.FromArgb("#3FC1B4"))
    {
        ...
        TintsSwitches = true,        // iOS: brand the switches too (green by default, as the HIG recommends)
    };
});
```
...
- `TintsSwitches` (iOS): switches use the brand color when on. It is off by default because the Human Interface Guidelines
  say to change a switch's color "only if necessary" (the default green works in most cases, the accent color is the
  accepted alternative). A `Switch` with its own `OnColor` keeps it. Android switches always follow the generated scheme.
